...
time_series_plots<-function(res=res, res.nogenetics=res.nogenetics){
  
  par(mfrow=c(3,2), mar=c(4,4,2,1))
  plot(0:10,res$fit.smolt[1:11]^2, type="l", ylim=c(0,1), ylab="Fitness", xlab="Generation")
  ...
  mtext(title, side=3, line=0.5, at=10.5, cex=0.75)
  
  y.upper.lim<-max(res$Sp.nat, na.rm=T)#res$Seq*res$mar.surv*40
  if(max(res$Sp.nat)>res$Seq*1.2){y.upper.lim<-max(res$Sp.nat)}
  
  plot(0:10, res$Sp.nat[1:11], type="l", col="red", ylim=c(0,y.upper.lim), ylab="Spawners (HOS+NOS)", xlab="Generation")
  points(x=0:10, y=res$NOS[1:11], col="red", type="l", lty="dashed")
  points(x=0:10, y=res$HOS[1:11], col="red", type="l",lty="dotted")
  points(x=0:10, y=res$BS[1:11], col="blue", type="l")
  points(x=0:10, y=res$NOB[1:11], col="blue", type="l",lty="dashed")
  points(x=0:10, y=res$HOB[1:11], col="blue", type="l",lty="dotted")
  ...
  plot(0:10, res$pHOSeff[1:11], type="l", col="red", ylim=c(0,1), ylab="Proportion (pHOSeff or pNOB)", xlab="Generation")
  lines(0:10, res$pNOB[1:11], col="blue")
  legend(x=5,y=0.5, legend=c("pHOSeff","pNOB"), cex=0.7, col=c("red", "blue"), lty=c("solid", "solid"), bty="n")
  text(x=1, y=0.93, labels="(c)")
  
  plot(0:10, res$PNI[1:11], type="l", col="black", ylim=c(0,1), ylab="PNI",  xlab="Generation")
  text(x=1, y=0.95, labels="(d)")
  
  plot(0:10, res$RperS[1:11], ylim=c(min(res$RperS)*0.8, max(res$RperS)*1.2), type="l", col="black",  ylab="Recruits/spawner on spawning grounds", xlab="Generation")
  ...
  plot(0:10, res$ret.nat.preharvest[1:11], type="l", col="red", ylim=c(0,y.upper.lim), ylab="Recruitment from HOS+NOS", xlab="Generation")
  points(x=0:10, res.nogenetics$ret.nat.preharvest[1:11], type="l", col="blue")
...
pdf(here::here("Results/FitnessPars/Timeseries-w2_100_hatchsize_0.3.pdf"))
hs <- 0.3
w <- sqrt(100)#sqrt(40)
#Theta.hatch = 80
